...
COPY locale.conf post-install.sh post-install.service vconsole.conf zram-generator.conf /tmp/sysconfig/
RUN mkdir -vp /var/opt /var/usrlocal /etc/sysusers.d /usr/lib/bootc/kargs.d /etc/modprobe.d && \
    ...
    systemctl enable thermald.service post-install.service spice-vdagentd.service && \
